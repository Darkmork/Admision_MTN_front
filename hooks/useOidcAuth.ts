...
import { Logger } from '../src/utils/logger';import { useOidc } from '../context/OidcContext';
import { Logger } from '../src/utils/logger';import type { MTNUser } from '../services/oidcService';
...
export const useTokenManager = () => {
  ...
  useEffect(() => {
    ...
    const checkTokenExpiry = () => {
      ...
      if (timeUntilExpiry <= fiveMinutes && tokenRenewalAttempts < 3) {
        Logger.info('⏰ Token próximo a expirar, renovando automáticamente...');
        ...
        renewToken()
          .then((newUser) => {
            if (newUser) {
              ...
              Logger.warn('⚠️ No se pudo renovar el token automáticamente');
              setTokenRenewalAttempts(prev => prev + 1);
            }
          })
          .catch((error) => {
            Logger.error('❌ Error renovando token automáticamente:', error);
            setTokenRenewalAttempts(prev => prev + 1);
          });
      }
    };
  ...
  const getValidToken = useCallback(async (): Promise<string | null> => {
    ...
    if (expiresAt - now <= oneMinute) {
      Logger.info('🔄 Token expirando pronto, renovando...');
...
export const useAuthRedirect = () => {
  ...
  const redirectIfNotAuthenticated = useCallback((fallbackPath: string = '/login') => {
    if (!authState.isLoading && !authState.isAuthenticated) {
      Logger.info('🔄 Usuario no autenticado, redirigiendo a:', fallbackPath);
  ...
  const redirectIfAuthenticated = useCallback((fallbackPath: string = '/dashboard') => {
    if (!authState.isLoading && authState.isAuthenticated) {
      Logger.info('🔄 Usuario ya autenticado, redirigiendo a:', fallbackPath);
      window.location.href = fallbackPath;
    }
  }, [authState.isAuthenticated, authState.isLoading]);
...
export const useAuthPersistence = () => {
  ...
  const getPersistedState = useCallback(() => {
    try {
      ...
      Logger.error('❌ Error leyendo estado persistido:', error);
      return null;
    }
  }, []);
